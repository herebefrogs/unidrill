 ...
 - **Music is rendered once at load, then only played back.** `renderSong()`
   (in `sound.js`) synthesises a whole voxby track to an `AudioBuffer` — it
   *blocks* ~0.1 s per call, so it runs in `onload` (game track immediately,
   title track a `setTimeout` tick later so they don't stack on one frame),
   never per-frame. Playback is a single looping `BufferSourceNode`.
   `updateMusic()` runs every frame but is a cheap no-op unless the wanted
   track differs from `musicBuffer`. The AudioContext starts suspended
   (autoplay policy) and is unlocked by a one-shot `keydown`/`pointerdown`
   listener that must call `resumeAudio()` *synchronously inside the gesture*
   — the `LOAD` screen exists to guarantee that gesture happens before
   gameplay. `toggleLoop()` suspends the context on pause/tab-hide (else the
   loop plays on over a hidden tab) and resumes on unpause.
