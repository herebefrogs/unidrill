 ...
 - **Two independent RNGs, never crossed.** `terrain.js` `hash2D` is a
   *stateless* pure hash — everything underground (terrain, dust, later
   ...
   by `setMapSeed(seed)` (terrain.js): one player-facing seed string folds
   through a local xfnv1a hash + a small LCG (self-contained, not imported
   from `utils.js`), and the LCG's first two outputs become `terrainSeed`/
   `dustSeed`. Still a pure fn of `(x,y)` *within a run*, so the stateless
   contract holds. `game.js`'s `seedMap()` resolves one `?seed=XXXXXXXX` URL
   param (no param → `JS13K2026`, uppercased otherwise) and writes it back to
   the URL for sharing; `rerollSeed()`'s **New seed** title-menu item draws a
   fresh 8-char uppercase (`randomSeed()`, straight `Math.random`, not
   `hash2D`/the prng stream). Seed 0 (unreachable from `seedMap`) = the old
   unseeded map. Deterministic spawn: `pickSpawnX()`
